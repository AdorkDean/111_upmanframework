	...
	*/
	...
	if(re.test(objForm)){  //有中文字符
		...
		if(num!=1){			
			...
			re = /\s*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/; 			
			return re.test(objForm);			
		}
	}else{
		re = /^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/; //没有中文字符的匹配串
		return re.test(objForm);
	}
}
...
function isValidMobileNo(strMobileNo) {
    ...
    //var re = /^\d*(\-){0,1}1\d{10}$/ ;
	//var re = /(^[0-9]{1,4}\-{0,1}1\d{10}$)|(^\([0-9]{1,4}\)1[0-9]{10}$)|(^1[0-9]{10}$)/;
	var re = /(^[0-9]{1,4}\-{0,1}1\d*$)|(^\([0-9]{1,4}\)1[0-9]*$)|(^1[0-9]*$)/;
    if (!re.test(strMobileNo)) {
		//alert("您输入的手机号包含数字和-以外的字符！");
        return false;
    }
	/*
	if( strMobileNo.length>16 || strMobileNo.length > 10){ //国际号从1位到4位不等
		alert("手机号位数不正确");
		reteurn false;
	}
...
function isValidUrl(url){
	...
	//return urlpatern.test(url.toLowerCase());
